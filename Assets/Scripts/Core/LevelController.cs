...
public class LevelController : MonoBehaviour
{
    ...
    public HexMapGenerator HexMapGenerator => hexMapGenerator;
    ...
    private void Awake()
    {
        ...
        if (hexMapGenerator == null)
            hexMapGenerator = GetComponent<HexMapGenerator>();

        if (hexMapGenerator == null)
            hexMapGenerator = gameObject.AddComponent<HexMapGenerator>();
        ...
        RectTransform regionRect = regionsContainer as RectTransform;
        if (regionRect != null && regionRect.GetComponent<HexMapCameraController>() == null)
            regionRect.gameObject.AddComponent<HexMapCameraController>();
        ...
        // Scene-level UI is now owned by GameplaySceneInstaller/editor rebuild tools.
        // LevelController keeps gameplay state only and no longer auto-spawns menu/save/character panels.
    }
    ...
    private void Start()
    {
        ...
        GenerateRegions();
        if (!GameSession.LoadRequested && CharacterManager.Instance != null)
            CharacterManager.Instance.GenerateForCurrentWorld();
        ...
        if (GameSession.LoadRequested && SaveLoadManager.Instance != null)
            SaveLoadManager.Instance.TryLoadGame(GameSession.RequestedLoadSlot);
    }
    ...
    void GenerateRegions()
    {
        ...
        if (hexMapGenerator != null)
        {
            hexMapGenerator.GenerateRegions();
    ...
    public void SetResources(int sanity, int money, int artifacts)
    {
        Sanity = Mathf.Clamp(sanity, 0, 100);
        Money = Mathf.Max(0, money);
        Artifacts = Mathf.Max(0, artifacts);
    ...
    {
        if (IsPointerOverRegionUI())
            return;

        if (IsPointerOverBlockingUI())
            return;

        if (!IsPointerOverWorldSurface())
            return;
    ...
    bool IsPointerOverBlockingUI()
    {
        ...
        for (int i = 0; i < raycastResults.Count; i++)
        {
            ...
            if (worldClickSurface != null &&
                (go.transform.IsChildOf(worldClickSurface) || worldClickSurface.transform.IsChildOf(go.transform)))
                continue;

            if (!IsInteractiveUIElement(go))
                continue;
    ...
    public void ClearRegionUIRegistry()
    {
    ...
    void ApplyDifficultySettings()
    {
        ...
        Sanity = difficulty.startingSanity;
        Money = difficulty.startingMoney;
        Artifacts = difficulty.startingArtifacts;
        sanityRegenRate = difficulty.sanityChangePerTenSeconds;
    }
}
