...
public class RegionModifier
{
    ...
    public RegionModifier(
        ...
        int influencePerTick,
        int stabilityPerTick,
        int developmentPerTick,
        float remainingDurationSeconds)
    {
        Name = string.IsNullOrEmpty(name) ? "Modifier" : name;
        DurationSeconds = Mathf.Max(0f, durationSeconds);
        TickIntervalSeconds = Mathf.Max(0.1f, tickIntervalSeconds <= 0f ? 1f : tickIntervalSeconds);
        InfluencePerTick = influencePerTick;
        StabilityPerTick = stabilityPerTick;
        DevelopmentPerTick = developmentPerTick;
        remainingDuration = remainingDurationSeconds < 0f ? Mathf.Infinity : Mathf.Max(0f, remainingDurationSeconds);
        tickAccumulator = 0f;
    }
