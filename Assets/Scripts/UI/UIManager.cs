...
public class UIManager : MonoBehaviour
{
    ...
    void ApplyRegionInfo(Region region)
    {
        ...
        if (nameText != null)
            nameText.text = $"{region.Name}\n{FormatRegionType(region.Type)} [{region.HexQ}, {region.HexR}]";
        if (influenceText != null)
    ...
    void UpdateModifierList(Region region)
    {
        ...
        {
            modifiersText.text = BuildRegionMetadata(region) + "\nModifiers: none\n" + BuildCharacterModifierText(region);
            return;
        }

        StringBuilder builder = new StringBuilder();
        builder.AppendLine(BuildRegionMetadata(region));
        ...
        builder.AppendLine();
        builder.Append(BuildCharacterModifierText(region));

        modifiersText.text = builder.ToString();
    }

    string BuildRegionMetadata(Region region)
    {
        if (region == null)
            return string.Empty;

        return $"ID: {region.Id}\nType: {FormatRegionType(region.Type)}\nNeighbors: {region.NeighborIds.Count}";
    }

    string BuildCharacterModifierText(Region region)
    {
        if (region == null || CharacterManager.Instance == null)
            return "Character modifiers: none";

        return CharacterManager.Instance.BuildRegionCharacterModifierSummary(region.Id);
    }

    string FormatRegionType(RegionType type)
    {
        switch (type)
        {
